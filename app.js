...
function setLastSync(value, cached) {
  ...
  el.textContent = (cached ? 'Önbellek: ' : 'Son yenileme: ') + new Date(value).toLocaleString('tr-TR');
}
...
function addRecentProduct(prod…2616 tokens truncated…an class="basket-qty"><button onclick="changeBasketQty(' + idx + ', -1)" aria-label="Adedi azalt">−</button>' +
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function(){ navigator.serviceWorker.register('service-worker.js?v=11').catch(function(){}); });
}
